...
# CONSTANTS & HELPERS
# =========================
MACHINES = ["Atlas Max", "Atlas Max Plus", "Atlas Max Poly", "Apollo"]  # Atlas* => 1 belt, Apollo => 4
UNIT_OPTIONS = ["°C", "°F"]
...
MACHINE_PRESETS = {
    ...
    "Apollo":          {"belts": 4, "durations": [4.5, 3.0, 3.0, 3.0],    "setpoints_F": [342, 305, 305, 305]},
}
...
def parse_sd_txt(text: str, filename: str) -> pd.DataFrame:
    ...
    for raw in text.splitlines():
        line = raw.strip()
        if not line or not line.startswith("AT"): continue
        toks = line.split()
        if len(toks) < 6: continue
        _, date, time, interval = toks[0], toks[1], toks[2], toks[3]
        unit = toks[-1]
        values = toks[4:-1]
...
def apply_machine_preset_for(machine_name: str):
    ...
    for i in range(n):
        ...
        st.session_state["belts_cfg"][b] = {"duration_min": dur, "setpoint_display": sp_disp, "color": DEFAULT_BELT_COLORS.get(b, "#888")}
...
preset_belts = MACHINE_PRESETS.get(machine, {"belts": 1})["belts"]
if machine == "Apollo":
    n_belts = st.number_input("Number of BELTs (Apollo only)", min_value=1, max_value=4,
                              value=int(st.session_state.get("n_belts", preset_belts)),
                              step=1, key="num_belts_apollo")
    ...
    for row_start in range(0, len(belt_labels), 2):
        ...
        for col, belt in zip(cols, row_belts):
            with col:
                ...
                belts_cfg[belt] = {"duration_min": float(dur), "setpoint_display": float(sp_disp), "color": belt_col}
    ...
    if "belts_cfg" not in st.session_state or "BELT 1" not in st.session_state["belts_cfg"]:
        sp_default = DEFAULT_SETPOINT_F if unit_display=="°F" else f_to_c(DEFAULT_SETPOINT_F)
        st.session_state["belts_cfg"] = {
            "BELT 1": {"duration_min": 4.5, "setpoint_display": float(sp_default), "color": DEFAULT_BELT_COLORS["BELT 1"]}
        }
...
if data_mode == "TXT (SD card)":
    ...
    uploaded_txt = st.file_uploader("Select one or more .TXT", type=["txt","TXT"], accept_multiple_files=True, key="upload_txt")
    if uploaded_txt:
        parsed=[]
    ...
    with c_top2:
        duration_min_manual = st.number_input("Total duration (min)", min_value=0.1, max_value=240.0, value=8.0, step=0.5, key="manual_duration_min")
    with c_top3:
    ...
    manual_meta = ("manual_S_headers", tuple(times), tuple(headers))
    ...
    long_df["source"] = f"{machine} (manual)"; long_df["unit"] = "C"
    frames.append(long_df[["source","elapsed_min","series","channel","temp_C","unit","side"]])
...
def header_page(machine: str,
    ...
    for b,cfg in belts_cfg.items():
        lines.append(f"  • {b}: Duration {cfg['duration_min']:.2f} min | SP {cfg['setpoint_display']:.1f}{unit_display.replace('°','')} | Color {cfg['color']}")
    lines.append("")
